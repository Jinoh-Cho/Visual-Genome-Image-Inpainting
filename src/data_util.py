...
class Dataset_(Dataset):
    def __init__(self,
        ...
        if self.hdf5_path is None:
            if crop_long_edge:
                crop_op = CenterCropLongEdge() if self.train else CenterCropLongEdge()
    ...
    def load_dataset(self):
        ...
        if self.data_name == "CIFAR10":
            ...
            source_root = os.path.join(self.data_dir, "train", "source")
            target_root = os.path.join(self.data_dir, "train", "target")
            self.source = ImageFolder(root=source_root)
            self.target = ImageFolder(root=target_root)
    ...
    def __getitem__(self, index):
        if self.hdf5_path is None:
            src_img, src_label = self.source[index]
            trg_img, trg_label = self.target[index]
        else:
            if self.load_data_in_memory:
                img, label = np.transpose(self.data[index], (1, 2, 0)), self.labels[index]
            else:
                img, label = self._get_hdf5(index)
        return self.trsf(src_img), self.trsf(trg_img), int(src_label), int(trg_label)
