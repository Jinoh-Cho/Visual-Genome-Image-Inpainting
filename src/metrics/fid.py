...
def calculate_moments(data_loader, generator, discriminator, eval_model, is_generate, num_generate, y_sampler,
    ...
    for i in tqdm(range(0, num_batches), disable=disable_tqdm):
        try:
            ...
            images_, images, labels_, labels = feed_list[0].to(device), feed_list[1].to(device), feed_list[2].to(device), feed_list[3].to(device)
...
def calculate_fid(data_loader,
    ...
    m2, s2 = calculate_moments(data_loader=data_loader,
                               generator=generator,
                               discriminator=discriminator,
                               eval_model=eval_model,
                               is_generate=True,
