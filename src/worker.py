...
class WORKER(object):
    ...
    def sample_data_basket(self):
        try:
            src_images, trg_images, src_labels, trg_labels = next(self.train_iter)
            ...
            self.train_iter = iter(self.train_dataloader)
            src_images, trg_images, src_labels, trg_labels = next(self.train_iter)
        return src_images, trg_images, src_labels, trg_labels
        ...
        for step_index in range(self.OPTIMIZATION.d_updates_per_step):
            ...
            for acml_index in range(self.OPTIMIZATION.acml_steps):
                with torch.cuda.amp.autocast() if self.RUN.mixed_precision and not self.is_stylegan else misc.dummy_context_mgr() as mpc:
                    ...
                    src_images, trg_images, src_labels, trg_labels = self.sample_data_basket()
                    src_images = src_images.to(self.local_rank, non_blocking=True)
                    trg_images = trg_images.to(self.local_rank, non_blocking=True)
                    src_labels = src_labels.to(self.local_rank, non_blocking=True)
                    trg_labels = trg_labels.to(self.local_rank, non_blocking=True)
                    ...
                    if self.LOSS.adv_loss == "MH":
                        ...
                        dis_acml_loss = self.LOSS.d_loss(real_dict["adv_output"],
                                                         fake_dict["adv_output"],
                                                         DDP=self.DDP)
                    ...
                    if self.LOSS.apply_cr:
                        real_prl_images = self.AUG.parallel_augment(trg_images)
                        real_prl_dict = self.Dis(real_prl_images, trg_labels)
                    ...
                    if self.LOSS.apply_gp:
                        gp_loss = losses.cal_grad_penalty(real_images=trg_images,
                                                          real_labels=trg_labels,
                    ...
                    if self.LOSS.apply_dra:
                        dra_loss = losses.cal_dra_penalty(real_images=trg_images,
                                                          real_labels=trg_labels,
                                                          discriminator=self.Dis,
                                                          device=self.local_rank)
                    ...
                    if self.LOSS.apply_maxgp:
                        maxgp_loss = losses.cal_maxgrad_penalty(real_images=trg_images,
                                                                real_labels=trg_labels,
                                                                fake_images=fake_images,
                                                                discriminator=self.Dis,
                                                                device=self.local_rank)
                    ...
                    dis_acml_loss = dis_acml_loss/1000
                    dis_acml_loss += dis_recon_loss
        ...
        for step_index in range(self.OPTIMIZATION.g_updates_per_step):
            ...
            for acml_step in range(self.OPTIMIZATION.acml_steps):
                with torch.cuda.amp.autocast() if self.RUN.mixed_precision and not self.is_stylegan else misc.dummy_context_mgr() as mpc:
                    ...
                    # calculate reconstruction loss
                    gen_recon_loss = self.l2_loss(trg_images, fake_images)
                    ...
                    # adjust gradients for applying gradient accumluation trick
                    gen_acml_loss = gen_acml_loss/1000
                    gen_acml_loss += gen_recon_loss
    ...
    def visualize_fake_images(self, num_cols, current_step):
        ...
        wandb.log({"source_images": wandb.Image(src_images)}, step=self.wandb_step)
        wandb.log({"target_images": wandb.Image(trg_images)}, step=self.wandb_step)
