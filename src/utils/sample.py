...
def generate_images(z_prior, truncation_factor, images, labels, batch_size, z_dim, num_classes, y_sampler, radius, generator,
                    discriminator, is_train, LOSS, RUN, device, is_stylegan, generator_mapping, generator_synthesis, style_mixing_p,
                    cal_trsp_cost):
    if is_train:
    ...
    ws, trsp_cost, fake_images_eps = None, None, None
    zs, _, zs_eps = sample_zy(z_prior=z_prior,
    ...
    fake_images = generator(zs, labels, images,  eval=not is_train)
    return fake_images, labels, fake_images_eps, trsp_cost, ws
